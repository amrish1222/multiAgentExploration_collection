...
class agentModelCNN2_Double(nn.Module):
...
class DoubleCNNagent():
    ...
    def buildModel(self,env):   
        ...
        self.model1 = agentModelCNN2_Double(env, self.device, self.loggingLevel).to(self.device)
        self.model2 = agentModelCNN2_Double(env, self.device, self.loggingLevel).to(self.device)
    ...
    def trainModel(self):
        ...
        for i in range(2): # number epoh
            ...
            loss1 = self.loss_fn1(Y1,predY1)
            loss2 = self.loss_fn2(Y2,predY2)
            loss1.backward()
            loss2.backward()
    ...
    def buildMiniBatchTrainData(self):
        ...
        self.trainY1 = Y1
        self.trainY2 = Y2
        return skMSE(Y1,qVal_c_1) + skMSE(Y2,qVal_c_2)
