...
#    print(f"episode, epsilon : {episode, dAgent.epsilon}")
    ...
    for step in range(LEN_EPISODE):
        ...
        dAction = []
        for ndx in range(NUM_DR):
            dAction.append(env.display.getInput())
        ...
        testDisp = list(copy.deepcopy(next_state))
        testDisp[2] = 0
        print(testDisp)
        print("drone inside :", np.any(next_state[2][0]==100), np.count_nonzero(next_state[2][0] == 255))
